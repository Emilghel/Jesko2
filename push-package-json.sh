#!/bin/bash
# Script to update package.json with the full dependencies list
# Usage: ./push-package-json.sh <github_token>

if [ -z "$1" ]; then
  echo "Please provide your GitHub token as the first argument"
  echo "Usage: ./push-package-json.sh <github_token>"
  exit 1
fi

GITHUB_TOKEN=$1
TEMP_DIR="/tmp/jesko-package-json"
...
# Copy the complete package.json file
...
# Add and commit
echo "Adding and committing updated package.json..."
git add package.json
git commit -m "Update package.json with complete dependencies"
git push origin main

echo "package.json updated on GitHub!"