#!/bin/bash
# Script to update the README.md with proper deployment instructions
# Usage: ./push-readme.sh <github_token>

if [ -z "$1" ]; then
  echo "Please provide your GitHub token as the first argument"
  echo "Usage: ./push-readme.sh <github_token>"
  exit 1
fi
...
rm -rf /tmp/jesko-readme
git clone https://${GITHUB_TOKEN}@github.com/Emilghel/Jesko2.git /tmp/jesko-readme
cd /tmp/jesko-readme
...
# Add and commit
echo "Adding and committing updated README..."
git add README.md
git commit -m "Update README with comprehensive deployment instructions"
