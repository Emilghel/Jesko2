#!/bin/bash
# Script to push the server/lib/auth-simple.ts file
# Usage: ./push-auth-simple.sh <github_token>

if [ -z "$1" ]; then
  echo "Please provide your GitHub token as the first argument"
  echo "Usage: ./push-auth-simple.sh <github_token>"
  exit 1
fi

GITHUB_TOKEN=$1
TEMP_DIR="/tmp/jesko-auth-simple"
...
# Copy the auth-simple.ts file from the current environment
echo "Copying auth-simple.ts from the current environment..."
...
# Add and commit
echo "Adding and committing auth-simple.ts..."
git add server/lib/auth-simple.ts
git commit -m "Add complete auth-simple.ts implementation"
git push origin main

echo "auth-simple.ts pushed to GitHub!"